...
当前存档 Schema 为 V2：`progress.highestStage` 记录历史最高关卡，`heroes` 固定保存 8 位英雄的 `heroId`、`level`、`isUnlocked` 与 `isDeployed`。V1 读取后逐级迁移到 V2，保留主角等级、金币和关卡，并按历史最高关卡补齐英雄解锁状态。为兼容现有玩家，本地 Storage 键暂时沿用 `ashrealm.save.v1`，键名不得作为 Schema 版本判断依据。
